...
$total_sql="SELECT SUM(amount) AS total FROM expenses WHERE user_id=$user_id";
$total_result=mysqli_query($conn,$total_sql);
$total_row=mysqli_fetch_assoc($total_result);
$total=$total_row['total'] ? $total_row['total'] : 0;

$count_sql="SELECT COUNT(*) AS count FROM expenses WHERE user_id=$user_id";
$count_result=mysqli_query($conn,$count_sql);
$count_row=mysqli_fetch_assoc($count_result);
$count=$count_row['count'];

$month_sql="SELECT SUM(amount) AS month_total FROM expenses WHERE user_id=$user_id AND MONTH(expense_date)=MONTH(CURDATE()) AND YEAR(expense_date)=YEAR(CURDATE())";
